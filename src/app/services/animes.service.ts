...
export class AnimesService {
   ...
   */
  private baseUrl = 'http://localhost:8090/ms';
  ...
  consultarAnimes(): Observable<Anime[]> {
    return this.httpClient.get<Anime[]>(`${this.baseUrl}/animes`);
  }
  ...
  guardarAnime(anime: Anime): Observable<Anime> {
    return this.httpClient.post<Anime>(`${this.baseUrl}/anime`, anime, {headers: this.headers});
  }
  ...
  actualizarAnime(anime: Anime): Observable<Anime> {
    return this.httpClient.put<Anime>(`${this.baseUrl}/anime`, anime, {headers: this.headers});
  }
  ...
  eliminarAnime(id: number): Observable<void> {
    return this.httpClient.delete<void>(`${this.baseUrl}/anime/${id}`);
  }
